...
function StorageTooltip({ active, payload, label }: { active?: boolean; payload?: Array<{ color?: string; name?: string; value?: number }>; label?: string }) {
  ...
  return (
    <div className="rounded-[8px] border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-3 text-[12px] font-semibold text-gray-900 dark:text-slate-100 shadow-sm card-depth">
      <p className="mb-2 font-mono text-[11px] font-normal text-gray-500 dark:text-slate-400">{label}</p>
...
export default function StorageMonitoring() {
  ...
  return (
    <div className="mx-auto w-full max-w-[1400px] px-3 md:px-6 pt-4 pb-8">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center justify-between rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 text-left shadow-sm card-depth">
        <div className="flex flex-1 items-center gap-3 w-full overflow-hidden">
          <div className="hidden md:block border-r border-gray-200 dark:border-slate-700 pr-5 text-[14px] font-black text-gray-900 dark:text-slate-100 whitespace-nowrap uppercase tracking-widest">Total {companies.length}</div>
          <select className="bg-white dark:bg-slate-900 h-[38px] flex-1 min-w-0 md:w-[220px] md:flex-none rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 text-sm font-bold text-gray-700 dark:text-slate-300 focus:border-blue-500 focus:outline-none transition-all">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <div className="flex h-[38px] flex-1 md:w-[180px] items-center gap-2 rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 shadow-sm card-depth overflow-hidden">
            <Calendar className="shrink-0 text-gray-400 dark:text-slate-500" size={14} />
          ...
          <button className="flex h-[38px] w-[38px] shrink-0 items-center justify-center rounded-lg border border-gray-200 dark:border-slate-700 text-gray-400 dark:text-slate-500 hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-900 shadow-sm card-depth transition-all">
            <RotateCcw size={16} />
          </button>
      ...
      {error ? (
        <div className="rounded-xl border border-rose-200 bg-rose-50 px-4 py-4 text-[13px] text-rose-700">
          {error}
        </div>
      ) : null}

      <div className="flex w-full min-w-0 flex-1 flex-col gap-6 overflow-y-auto pr-1">
        {loading ? (
          <div className="rounded-[10px] border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6 text-[14px] text-gray-400 dark:text-slate-500">
            스토리지 데이터를 불러오는 중입니다...
          </div>
        ) : (
          companies.map((company) => (
            <div key={company.tenantId} className="overflow-hidden rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-left shadow-sm card-depth">
              <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-gray-100 dark:border-slate-700 bg-[#FAFAFA] dark:bg-slate-900/50 p-5 gap-4">
                <div className="flex items-center gap-4">
                  <h2 className="text-[15px] font-black text-gray-900 dark:text-slate-100 uppercase tracking-tight">{company.name}</h2>
                  <span className="font-mono text-[11px] font-bold text-gray-400 dark:text-slate-500 bg-gray-100 px-2 py-0.5 rounded flex items-center">{company.tenantId}</span>
                </div>
                <div className="flex flex-wrap items-center gap-3 rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-4 py-2">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-black uppercase tracking-widest text-gray-400 dark:text-slate-500">USAGE</span>
                    <span className="font-mono text-sm font-black text-gray-900 dark:text-slate-100 leading-none">
              ...
              <div className="p-6">
                <div className="mb-6">
                  <div className="mb-3 flex items-center justify-between">
                    <h3 className="text-[13px] font-semibold text-gray-900 dark:text-slate-100">Capacity (TB)</h3>
                    <MoreVertical className="cursor-pointer text-gray-400 dark:text-slate-500" size={16} />
                  </div>
                ...
                <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                  <div>
                    <div className="mb-3 flex items-center justify-between">
                      <h3 className="text-[13px] font-semibold text-gray-900 dark:text-slate-100">Bandwidth (Mbps)</h3>
                      <MoreVertical className="cursor-pointer text-gray-400 dark:text-slate-500" size={16} />
                    </div>
                  ...
                  <div>
                    <div className="mb-3 flex items-center justify-between">
                      <h3 className="text-[13px] font-semibold text-gray-900 dark:text-slate-100">IOPS</h3>
                      <MoreVertical className="cursor-pointer text-gray-400 dark:text-slate-500" size={16} />
                    </div>
                    <div className="h-[180px] w-full min-w-0">
                      <ResponsiveContainer height="100%" width="100%">
                  ...
                  <div>
                    <div className="mb-3 flex items-center justify-between">
                      <h3 className="text-[13px] font-semibold text-gray-900 dark:text-slate-100">Latency (ms)</h3>
                      <MoreVertical className="cursor-pointer text-gray-400 dark:text-slate-500" size={16} />
                    </div>
                    <div className="h-[180px] w-full min-w-0">
                      <ResponsiveContainer height="100%" width="100%">
