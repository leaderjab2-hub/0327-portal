    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-gray-950/45 p-4 backdrop-blur-sm">
      <div className="flex w-full max-w-2xl flex-col overflow-hidden rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-2xl">
        <div className="flex items-center justify-between border-b border-gray-100 dark:border-slate-700 px-6 py-4">
          <h2 className="text-xl font-black text-gray-900 dark:text-slate-100 tracking-tight">{title}</h2>
          <button className="rounded-full p-2 text-gray-400 dark:text-slate-500 hover:bg-gray-100 hover:text-gray-900 dark:text-slate-100 transition-all" onClick={onClose}><X size={20} /></button>
        </div>
        <div className="flex-1 overflow-y-auto p-6 scroll-smooth bg-gray-50 dark:bg-slate-900/50">{children}</div>
      </div>
    </div>
  );
}
    ...
    <div className="flex h-full flex-col bg-[#F8FAFC] dark:bg-slate-900">
      <div className="flex-1 overflow-y-auto w-full">
        <div className="mx-auto w-full max-w-[1400px] px-6 pb-8 space-y-6">
          <div className="flex h-[48px] shrink-0 items-center justify-between bg-white dark:bg-slate-800 px-4 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm card-depth overflow-hidden">
            <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-hide py-1 min-w-0 flex-1">
              {filteredTenants.map((t) => {
                ...
                return (
                  <button key={t.id} onClick={() => setSelectedTenantId(t.id)} className={`whitespace-nowrap rounded-full px-4 py-1.5 text-sm font-bold transition-all ${isSelected ? 'bg-primary-50 text-blue-600' : 'text-gray-500 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-900'}`}>
                    {t.name}
                  </button>
                );
              })}
            ...
            <div className="hidden md:flex items-center gap-4 pl-4 shrink-0">
              ...
              <div className="relative shrink-0 flex items-center gap-2">
                ...
                <input className="bg-white dark:bg-slate-900 h-[30px] w-36 rounded-full border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 pl-8 pr-4 text-[12px] transition-all focus:w-48 focus:border-blue-300 focus:outline-none" placeholder="회사 검색" value={tenantSearch} onChange={e => setTenantSearch(e.target.value)} />
                {editable && (
          ...
          {selectedTenant ? (
            <>
              <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm card-depth p-4 md:p-6 space-y-4 md:space-y-6">
                <div className="flex items-start justify-between">
                  <div className="space-y-3 md:space-y-4">
                    <h2 className="text-lg md:text-2xl font-black text-gray-900 dark:text-slate-100 tracking-tight">{selectedTenant.name} <span className="text-[10px] md:text-sm font-bold text-gray-400 dark:text-slate-500 ml-1 md:ml-2 italic">#{selectedTenant.id}</span></h2>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-6">
                      <div className="space-y-0.5 md:space-y-1">
                        <p className="text-[9px] md:text-[10px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest flex items-center gap-1.5"><Calendar size={11} className="shrink-0"/> 계약 기간</p>
                        <p className="text-[11px] md:text-sm font-bold text-blue-600">{selectedTenant.contract.startDate || "-"} ~ {selectedTenant.contract.endDate || "-"}</p>
                      </div>
                      <div className="space-y-0.5 md:space-y-1">
                        <p className="text-[9px] md:text-[10px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest flex items-center gap-1.5"><UserCheck size={11} className="shrink-0"/> 담당 정보</p>
                        <p className="text-[11px] md:text-sm font-bold text-gray-700 dark:text-slate-300 truncate max-w-[200px]">{selectedTenant.contractorEmail || "-"} / {selectedTenant.managerEmail || "-"}</p>
                      </div>
                      <div className="hidden sm:block space-y-0.5 md:space-y-1">
                        <p className="text-[9px] md:text-[10px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest">계약 생성일</p>
                        <p className="text-[11px] md:text-sm font-bold text-gray-500 dark:text-slate-400 italic">{selectedTenant.createdAt}</p>
                      </div>
                    </div>
                  </div>
                  {editable && (
                    <div className="flex gap-2">
                      <button className="p-2.5 rounded-lg border border-gray-200 dark:border-slate-700 text-gray-500 dark:text-slate-400 hover:text-blue-600 hover:bg-blue-50 transition-all active:scale-[0.95]" onClick={() => setTenantEditor({ open: true, mode: "edit", form: { id: selectedTenant.id, name: selectedTenant.name, contractorEmail: selectedTenant.contractorEmail, managerEmail: selectedTenant.managerEmail, startDate: selectedTenant.contract.startDate || "", endDate: selectedTenant.contract.endDate || "", gpuQuantity: String(selectedTenant.contract.gpu?.quantity || ""), gpuUnitPrice: String(selectedTenant.contract.gpu?.unitPrice || ""), cpuQuantity: String(selectedTenant.contract.cpu?.quantity || ""), cpuUnitPrice: String(selectedTenant.contract.cpu?.unitPrice || ""), storageCapacity: String(selectedTenant.contract.storage?.capacity || ""), storageUnit: selectedTenant.contract.storage?.unit || "TB", storageUnitPrice: String(selectedTenant.contract.storage?.unitPrice || ""), networkBandwidth: String(selectedTenant.contract.network?.bandwidth || ""), networkUnit: selectedTenant.contract.network?.unit || "Gbps", networkUnitPrice: String(selectedTenant.contract.network?.unitPrice || "") } })}><Pencil size={16} /></button>
                      {isAdmin && <button className="p-2.5 rounded-lg border border-gray-200 dark:border-slate-700 text-gray-400 dark:text-slate-500 hover:text-red-600 hover:bg-red-50 transition-all active:scale-[0.95]" onClick={async () => { if(confirm(`${selectedTenant.name}을 삭제할까요?`)) { await fetch(`/api/tenants/${selectedTenant.id}`, {method:'DELETE'}); loadContracts(); }}}><Trash2 size={16} /></button>}
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-end">
                    <span className="text-[10px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest">계약 이행률 (SLA)</span>
              ...
              <div className="grid grid-cols-3 gap-3 md:gap-4">
                <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm card-depth p-3 md:p-5 transition-all hover:shadow-md">
                   <div className="flex items-center gap-2 mb-1.5 md:mb-3">
                      <div className="w-6 h-6 md:w-8 md:h-8 rounded-lg bg-blue-50 flex items-center justify-center"><Package size={14} className="md:size-[16px] text-blue-500" /></div>
                      <span className="text-[10px] md:text-sm text-gray-500 dark:text-slate-400 font-bold uppercase tracking-wider truncate">Project</span>
                   </div>
                   <p className="text-base md:text-2xl font-black text-gray-900 dark:text-slate-100 tabular-nums">{selectedTenant.subtenants.length}<span className="text-[10px] md:text-sm ml-0.5 md:ml-1 text-gray-400 dark:text-slate-500">개</span></p>
                </div>
                <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm card-depth p-3 md:p-5 transition-all hover:shadow-md">
                   <div className="flex items-center gap-2 mb-1.5 md:mb-3">
                      <div className="w-6 h-6 md:w-8 md:h-8 rounded-lg bg-emerald-50 flex items-center justify-center"><Server size={14} className="md:size-[16px] text-emerald-500" /></div>
                      <span className="text-[10px] md:text-sm text-gray-500 dark:text-slate-400 font-bold uppercase tracking-wider truncate">Total GPU</span>
                   </div>
                   <p className="text-base md:text-2xl font-black text-gray-900 dark:text-slate-100 tabular-nums">{selectedTenant.contract.gpu?.quantity || 0}<span className="text-[10px] md:text-sm ml-0.5 md:ml-1 text-gray-400 dark:text-slate-500">장</span></p>
                </div>
                <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm card-depth p-3 md:p-5 transition-all hover:shadow-md">
                   <div className="flex items-center gap-2 mb-1.5 md:mb-3">
                      <div className="w-6 h-6 md:w-8 md:h-8 rounded-lg bg-amber-50 flex items-center justify-center"><HardDrive size={14} className="md:size-[16px] text-amber-500" /></div>
                      <span className="text-[10px] md:text-sm text-gray-500 dark:text-slate-400 font-bold uppercase tracking-wider truncate">Storage</span>
                   </div>
                   <p className="text-base md:text-2xl font-black text-gray-900 dark:text-slate-100 tabular-nums">{selectedTenant.contract.storage?.capacity || 0}<span className="text-[10px] md:text-sm ml-0.5 md:ml-1 text-gray-400 dark:text-slate-500 uppercase">{selectedTenant.contract.storage?.unit || 'TB'}</span></p>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm card-depth overflow-hidden">
                <div className="px-6 py-5 border-b border-gray-100 dark:border-slate-700 flex justify-between items-center bg-gray-50 dark:bg-slate-900/30">
                    <div>
                       <h3 className="text-sm font-black text-gray-900 dark:text-slate-100 uppercase tracking-widest">프로젝트 리스트</h3>
                       <p className="hidden md:block text-[11px] font-bold text-gray-400 dark:text-slate-500 mt-0.5">상세 계약 및 SLA가 적용되는 Subtenant 목록</p>
                    </div>
                    <div className="flex items-center gap-2 md:gap-3">
                       <div className="relative">
                          <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-slate-500" size={13} />
                          <input className="bg-white dark:bg-slate-900 h-9 w-24 sm:w-44 rounded-full border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 pl-8 pr-4 text-xs font-bold transition-all focus:w-32 sm:focus:w-56 outline-none" placeholder="프로젝트명" value={projectSearch} onChange={e => setProjectSearch(e.target.value)} />
                       </div>
                       {editable && (
                         <button className="flex items-center gap-1.5 px-3 md:px-4 py-2 bg-green-500 hover:bg-green-600 text-white text-[12px] md:text-sm font-black rounded-lg shadow-sm card-depth transition-all active:scale-[0.98] shrink-0" onClick={() => setSubtenantEditor({ open: true, mode: "create", form: defaultSubtenantForm, targetId: null })}>
                            <Plus size={14} /> <span className="hidden sm:inline">NEW PROJECT</span><span className="sm:hidden">ADD</span>
                         </button>
                       )}
                    </div>
                </div>
                <div className="overflow-x-auto">
                   <div className="md:hidden space-y-3 p-4 bg-gray-50 dark:bg-slate-900/50">
                      {loading ? (
                         <div className="py-20 text-center text-sm text-gray-400 dark:text-slate-500 italic">로딩 중...</div>
                      ) : filteredSubtenants.length === 0 ? (
                         <div className="py-20 text-center text-sm text-gray-400 dark:text-slate-500 italic">등록된 프로젝트가 없습니다.</div>
                      ) : (
                         filteredSubtenants.map(s => (
                            <div key={s.id} className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm card-depth p-4 space-y-4">
                               <div className="flex justify-between items-start">
                                  <div>
                                     <p className="text-sm font-black text-gray-900 dark:text-slate-100">{s.name}</p>
                                     <p className="text-[10px] font-bold text-gray-400 dark:text-slate-500 font-mono italic">ID: {s.id}</p>
                                  </div>
                                  <span className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-tighter border ${s.status === '활성' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : s.status === '대기' ? 'bg-amber-50 text-amber-700 border-amber-100' : 'bg-gray-100 text-gray-600 border-gray-200 dark:bg-slate-600 dark:text-slate-100 dark:border-slate-500'}`}>{s.status}</span>
                               </div>
                               <div className="flex flex-wrap gap-1">
                                  {s.products.map(p => <span key={p} className="px-1.5 py-0.5 rounded bg-gray-100 text-[10px] font-bold text-gray-500 border border-gray-200 dark:bg-slate-600 dark:text-slate-100 dark:border-slate-500">{p}</span>)}
                               </div>
                               <div className="pt-2 border-t border-gray-50 flex justify-between items-center bg-gray-50 dark:bg-slate-900/50 -mx-4 px-4 py-2 mt-2">
                                  <div className="space-y-1">
                                     <p className="text-[9px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest">계약 기간 / PM</p>
                                     <p className="text-[11px] font-bold text-gray-500 dark:text-slate-400">{s.startDate} ~ {s.endDate} / {s.pm}</p>
                                  </div>
                                  {editable && (
                                     <div className="flex gap-1">
                                        ...
                                        <button className="p-2 rounded-lg text-gray-400 dark:text-slate-500 hover:text-red-500 hover:bg-red-50 transition-all" onClick={async () => { if(confirm(`${s.name}을 삭제할까요?`)) { await fetch(`/api/subtenants/${s.id}`, {method:'DELETE'}); loadContracts(); }}}><Trash2 size={14} /></button>
                                     </div>
                                  )}
                               </div>
                            </div>
                   ...
                   <table className="hidden md:table w-full">
                      <thead>
                        <tr className="border-b border-gray-200 dark:border-slate-700">
                          <th className="px-6 py-4 text-[11px] font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider text-left bg-gray-50 dark:bg-slate-900/50">프로젝트 정보</th>
                          ...
                          <th className="px-6 py-4 text-[11px] font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider text-left bg-gray-50 dark:bg-slate-900/50">적용 상품</th>
                          <th className="px-6 py-4 text-[11px] font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider text-left bg-gray-50 dark:bg-slate-900/50 whitespace-nowrap">계약 기간</th>
                          <th className="px-6 py-4 text-[11px] font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider text-left bg-gray-50 dark:bg-slate-900/50">PM</th>
                          {editable && <th className="px-6 py-4 text-[11px] font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider text-center bg-gray-50 dark:bg-slate-900/50">관리</th>}
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-100">
                        {loading ? <tr><td colSpan={7} className="py-20 text-center text-sm text-gray-400 dark:text-slate-500 italic">로딩 중...</td></tr> : filteredSubtenants.length === 0 ? <tr><td colSpan={7} className="py-20 text-center text-sm text-gray-400 dark:text-slate-500 italic">등록된 프로젝트가 없습니다.</td></tr> : 
                          filteredSubtenants.map(s => (
                            <tr key={s.id} className="hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-900 transition-all">
                              <td className="px-6 py-4">
                                 <p className="text-sm font-black text-gray-900 dark:text-slate-100 group-hover:text-blue-600 transition-all">{s.name}</p>
                                 <p className="text-[10px] font-bold text-gray-400 dark:text-slate-500 font-mono italic">ID: {s.id}</p>
                              </td>
                              <td className="px-6 py-4">
                                <span className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-tighter border ${s.status === '활성' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : s.status === '대기' ? 'bg-amber-50 text-amber-700 border-amber-100' : 'bg-gray-100 text-gray-600 border-gray-200 dark:bg-slate-600 dark:text-slate-100 dark:border-slate-500'}`}>{s.status}</span>
                              ...
                              <td className="px-6 py-4 text-[11px] font-bold text-gray-500 dark:text-slate-400 font-mono tracking-tight">{s.startDate} ~ {s.endDate}</td>
                              <td className="px-6 py-4 text-xs font-bold text-gray-700 dark:text-slate-300">{s.pm}</td>
                              {editable && (
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-24 text-center space-y-4">
               <Building2 className="mx-auto text-gray-100" size={64} />
               <p className="text-lg font-black text-gray-400 dark:text-slate-500">Tenant를 선택해 주세요.</p>
            </div>
          )}
        </div>
      </div>
        ...
        <ModalFrame title={tenantEditor.mode === 'create' ? '새로운 Tenant 등록' : 'Tenant 정보 수정'} onClose={() => setTenantEditor(p => ({ ...p, open: false }))}>
           <div className="space-y-6">
              <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 p-5 shadow-sm card-depth space-y-4">
                 <h3 className="text-[11px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest border-l-4 border-blue-500 pl-3">기본 정보</h3>
                 <div className="grid grid-cols-2 gap-4">
                    <label className="space-y-1.5"><span className="text-[10px] font-black text-gray-500 dark:text-slate-400 uppercase">Tenant ID</span><input className="bg-white dark:bg-slate-900 w-full rounded-lg border border-gray-200 dark:border-slate-700 p-2 text-[13px] font-bold focus:border-blue-500 outline-none" disabled={tenantEditor.mode === 'edit'} value={tenantEditor.form.id} onChange={e => setTenantEditor(p => ({ ...p, form: { ...p.form, id: e.target.value } }))} /></label>
                    <label className="space-y-1.5"><span className="text-[10px] font-black text-gray-500 dark:text-slate-400 uppercase">회사명</span><input className="bg-white dark:bg-slate-900 w-full rounded-lg border border-gray-200 dark:border-slate-700 p-2 text-[13px] font-bold focus:border-blue-500 outline-none" value={tenantEditor.form.name} onChange={e => setTenantEditor(p => ({ ...p, form: { ...p.form, name: e.target.value } }))} /></label>
                    <label className="space-y-1.5"><span className="text-[10px] font-black text-gray-500 dark:text-slate-400 uppercase">계약자 이메일</span><input className="bg-white dark:bg-slate-900 w-full rounded-lg border border-gray-200 dark:border-slate-700 p-2 text-[13px] font-bold focus:border-blue-500 outline-none" value={tenantEditor.form.contractorEmail} onChange={e => setTenantEditor(p => ({ ...p, form: { ...p.form, contractorEmail: e.target.value } }))} /></label>
                    <label className="space-y-1.5"><span className="text-[10px] font-black text-gray-500 dark:text-slate-400 uppercase">담당자 이메일</span><input className="bg-white dark:bg-slate-900 w-full rounded-lg border border-gray-200 dark:border-slate-700 p-2 text-[13px] font-bold focus:border-blue-500 outline-none" value={tenantEditor.form.managerEmail} onChange={e => setTenantEditor(p => ({ ...p, form: { ...p.form, managerEmail: e.target.value } }))} /></label>
                 </div>
              </div>
              <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 p-5 shadow-sm card-depth space-y-4">
                 <h3 className="text-[11px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest border-l-4 border-emerald-500 pl-3">계약 기간</h3>
                 <div className="grid grid-cols-2 gap-4">
                    <label className="space-y-1.5"><span className="text-[10px] font-black text-gray-500 dark:text-slate-400 uppercase">시작일</span><input className="bg-white dark:bg-slate-900 w-full rounded-lg border border-gray-200 dark:border-slate-700 p-2 text-[13px] font-bold focus:border-blue-500 outline-none" type="date" value={tenantEditor.form.startDate} onChange={e => setTenantEditor(p => ({ ...p, form: { ...p.form, startDate: e.target.value } }))} /></label>
                    <label className="space-y-1.5"><span className="text-[10px] font-black text-gray-500 dark:text-slate-400 uppercase">종료일</span><input className="bg-white dark:bg-slate-900 w-full rounded-lg border border-gray-200 dark:border-slate-700 p-2 text-[13px] font-bold focus:border-blue-500 outline-none" type="date" value={tenantEditor.form.endDate} onChange={e => setTenantEditor(p => ({ ...p, form: { ...p.form, endDate: e.target.value } }))} /></label>
                 </div>
              </div>
              <div className="flex justify-end gap-3 pt-4">
                 <button className="px-5 py-2 rounded-lg text-sm font-bold text-gray-500 dark:text-slate-400 hover:bg-gray-100" onClick={() => setTenantEditor(p => ({ ...p, open: false }))}>취소</button>
      ...
      {subtenantEditor.open && (
        <ModalFrame title={subtenantEditor.mode === 'create' ? '새 프로젝트 생성' : '프로젝트 정보 수정'} onClose={() => setSubtenantEditor(p => ({ ...p, open: false }))}>
           <div className="space-y-6">
              <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 p-5 shadow-sm card-depth space-y-4">
                 <h3 className="text-[11px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest border-l-4 border-blue-500 pl-3">Subtenant Detail</h3>
                 <div className="grid grid-cols-2 gap-4">
                    <label className="space-y-1.5"><span className="text-[10px] font-black text-gray-500 dark:text-slate-400 uppercase">Subtenant ID</span><input className="bg-white dark:bg-slate-900 w-full rounded-lg border border-gray-200 dark:border-slate-700 p-2 text-[13px] font-bold focus:border-blue-500 outline-none" disabled={subtenantEditor.mode === 'edit'} value={subtenantEditor.form.id} onChange={e => setSubtenantEditor(p => ({ ...p, form: { ...p.form, id: e.target.value } }))} /></label>
                    <label className="space-y-1.5"><span className="text-[10px] font-black text-gray-500 dark:text-slate-400 uppercase">프로젝트명</span><input className="bg-white dark:bg-slate-900 w-full rounded-lg border border-gray-200 dark:border-slate-700 p-2 text-[13px] font-bold focus:border-blue-500 outline-none" value={subtenantEditor.form.name} onChange={e => setSubtenantEditor(p => ({ ...p, form: { ...p.form, name: e.target.value } }))} /></label>
                    <label className="space-y-1.5"><span className="text-[10px] font-black text-gray-500 dark:text-slate-400 uppercase">상태</span><select className="bg-white dark:bg-slate-900 w-full rounded-lg border border-gray-200 dark:border-slate-700 p-2 text-[13px] font-bold focus:border-blue-500 outline-none" value={subtenantEditor.form.status} onChange={e => setSubtenantEditor(p => ({ ...p, form: { ...p.form, status: e.target.value } }))}><option value="활성">활성</option><option value="대기">대기</option><option value="종료">종료</option></select></label>
                    <label className="space-y-1.5"><span className="text-[10px] font-black text-gray-500 dark:text-slate-400 uppercase">PM 이메일</span><input className="bg-white dark:bg-slate-900 w-full rounded-lg border border-gray-200 dark:border-slate-700 p-2 text-[13px] font-bold focus:border-blue-500 outline-none" value={subtenantEditor.form.pm} onChange={e => setSubtenantEditor(p => ({ ...p, form: { ...p.form, pm: e.target.value } }))} /></label>
                    <label className="col-span-2 space-y-1.5"><span className="text-[10px] font-black text-gray-500 dark:text-slate-400 uppercase">상품 목록 (콤마 구분)</span><input className="bg-white dark:bg-slate-900 w-full rounded-lg border border-gray-200 dark:border-slate-700 p-2 text-[13px] font-bold focus:border-blue-500 outline-none" placeholder="GPU 연산, 테라 스토리지 등" value={subtenantEditor.form.products} onChange={e => setSubtenantEditor(p => ({ ...p, form: { ...p.form, products: e.target.value } }))} /></label>
              ...
              <div className="flex justify-end gap-3 pt-4">
                 <button className="px-5 py-2 rounded-lg text-sm font-bold text-gray-500 dark:text-slate-400 hover:bg-gray-100" onClick={() => setSubtenantEditor(p => ({ ...p, open: false }))}>취소</button>
