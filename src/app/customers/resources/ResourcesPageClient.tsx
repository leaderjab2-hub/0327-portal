...
function RangeModal({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-sm">
      <div className="flex w-full max-w-lg flex-col overflow-hidden rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-xl">
        <div className="flex items-center justify-between border-b border-gray-100 dark:border-slate-700 px-6 py-4">
          ...
          <button className="rounded-full p-1.5 text-gray-400 dark:text-slate-500 transition hover:bg-gray-100 hover:text-gray-700 dark:text-slate-300" onClick={onClose} type="button">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="space-y-6 p-6">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              <label className="mb-2 block text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">시작 노드</label>
              <select 
                className="h-11 w-full rounded-lg border border-gray-200 dark:border-slate-700 px-4 text-sm focus:outline-none focus:border-blue-500 transition-all" 
            ...
            <div>
              <label className="mb-2 block text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">종료 노드</label>
              <select 
                className="h-11 w-full rounded-lg border border-gray-200 dark:border-slate-700 px-4 text-sm focus:outline-none focus:border-blue-500 transition-all" 
                value={safeEndNodeId} 
          ...
          <div className="rounded-xl border border-gray-100 dark:border-slate-700 bg-gray-50 dark:bg-slate-900/50 p-5">
            <div className="mb-2 text-xs font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider">선택된 리소스 범위</div>
        ...
        <div className="flex justify-end gap-3 bg-gray-50 dark:bg-slate-900 border-t border-gray-100 dark:border-slate-700 px-6 py-4">
          <button 
            className="px-5 py-2.5 rounded-lg text-sm font-bold text-gray-600 dark:text-slate-400 hover:bg-white dark:bg-slate-800 transition-all" 
            onClick={onClose} 
            type="button"
          >
            취소
          ...
          <button
            className={`px-6 py-2.5 rounded-lg text-sm font-black uppercase tracking-wider text-white shadow-sm card-depth transition-all disabled:bg-gray-300 ${
...
function ReclaimModal({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-sm">
      <div className="flex w-full max-w-xl flex-col overflow-hidden rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-xl">
        <div className="flex items-center justify-between border-b border-gray-100 dark:border-slate-700 px-6 py-4">
          <h2 className="text-lg font-bold text-gray-900 dark:text-slate-100">{title}</h2>
          <button className="rounded-full p-1.5 text-gray-400 dark:text-slate-500 transition hover:bg-gray-100 hover:text-gray-700 dark:text-slate-300" onClick={onClose} type="button">
            <X size={20} />
        ...
        <div className="space-y-6 p-6">
          <div className="grid gap-3 md:grid-cols-[1fr_1fr_auto]">
            <div>
              <label className="mb-2 block text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">시작 노드</label>
              <select className="bg-white dark:bg-slate-900 h-10 w-full rounded-lg border border-gray-200 dark:border-slate-700 px-3 text-sm" value={safeStartNodeId} onChange={(event) => setStartNodeId(event.target.value)}>
                {sortedAllocations.map((allocation) => (
            ...
            <div>
              <label className="mb-2 block text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">종료 노드</label>
              <select className="bg-white dark:bg-slate-900 h-10 w-full rounded-lg border border-gray-200 dark:border-slate-700 px-3 text-sm" value={safeEndNodeId} onChange={(event) => setEndNodeId(event.target.value)}>
                {sortedAllocations.map((allocation) => (
                  <option key={allocation.id} value={allocation.node_id}>
                    {toNodeChipLabel(allocation.node_id)}
                  </option>
            ...
            <div className="flex items-end">
              <button 
                className="h-10 px-4 rounded-lg bg-gray-100 text-xs font-bold text-gray-700 dark:text-slate-300 hover:bg-gray-200 transition-all" 
                onClick={selectRange} 
                type="button"
              >
                범위 추가
          ...
          <div className="rounded-xl border border-gray-100 dark:border-slate-700 bg-gray-50 dark:bg-slate-900/50 p-5">
            <div className="mb-3 flex items-center justify-between">
              <div className="text-xs font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider">회수 대상 노드 목록</div>
            ...
            <div className="grid max-h-[250px] gap-2 overflow-y-auto sm:grid-cols-2 p-1">
              {sortedAllocations.map((allocation) => (
                <label key={allocation.id} className={`flex cursor-pointer items-center gap-3 rounded-lg border p-3 transition-all ${
                  selectedIds.includes(allocation.id) ? 'bg-red-50 border-red-100' : 'bg-white dark:bg-slate-800 border-gray-100 dark:border-slate-700 hover:border-gray-200 dark:border-slate-700'
                }`}>
                  <input 
                    ...
                    type="checkbox" 
                  />
                  <span className="font-mono text-[13px] font-bold text-gray-900 dark:text-slate-100">{toNodeChipLabel(allocation.node_id)}</span>
                </label>
              ))}
            </div>
          </div>
        ...
        <div className="flex justify-between gap-3 bg-gray-50 dark:bg-slate-900 border-t border-gray-100 dark:border-slate-700 px-6 py-4">
          <button
            className="px-5 py-2.5 rounded-lg text-sm font-bold text-red-600 hover:bg-red-50 transition-all"
          ...
          <div className="flex gap-2">
            <button className="px-5 py-2.5 rounded-lg text-sm font-bold text-gray-600 dark:text-slate-400 hover:bg-white dark:bg-slate-800" onClick={onClose} type="button">
              취소
            </button>
            <button
              className="px-6 py-2.5 rounded-lg bg-red-600 text-sm font-black uppercase tracking-wider text-white shadow-sm card-depth hover:bg-red-700 disabled:opacity-40 transition-all"
    ...
    <div className="flex h-full flex-col bg-[#F8FAFC] dark:bg-slate-900">
      <div className="flex-1 overflow-y-auto w-full">
        <div className="mx-auto w-full max-w-[1400px] px-6 pb-8 space-y-6">
                ...
                return (
                  <button
                    ...
                    className={`whitespace-nowrap rounded-full px-4 py-1.5 text-sm font-bold transition-all ${
                      isSelected ? 'bg-blue-50 text-blue-600' : 'text-gray-500 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-900'
                    }`}
                  >
                    {c.name}
                  </button>
          ...
          {loading ? (
            <div className="flex h-[400px] items-center justify-center text-sm text-gray-400 dark:text-slate-500 italic">로딩 중...</div>
          ) : error ? (
            ...
            <div className="space-y-6">
               <div className="grid grid-cols-3 gap-3 md:gap-4">
                {selectedTenant ? (
                  <>
                    <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm card-depth p-3 md:p-5 transition-all hover:shadow-md">
                      <div className="flex items-center gap-2 mb-1.5 md:mb-3">
                        <div className="w-6 h-6 md:w-8 md:h-8 rounded-lg bg-blue-50 flex items-center justify-center">
                          <Package size={14} className="md:size-[16px] text-blue-500" />
                        </div>
                        <span className="text-[10px] md:text-sm text-gray-500 dark:text-slate-400 font-bold uppercase tracking-tight truncate">계약 수량</span>
                      </div>
                      <p className="text-base md:text-2xl font-black text-gray-900 dark:text-slate-100 tabular-nums">{selectedTenant.contractCount}<span className="text-[10px] md:text-sm ml-0.5 md:ml-1 text-gray-400 dark:text-slate-500">대</span></p>
                      <p className="hidden md:block text-xs text-gray-400 dark:text-slate-500 mt-1">Tenant 배정 목표</p>
                    </div>
                    <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm card-depth p-3 md:p-5 transition-all hover:shadow-md">
                      <div className="flex items-center gap-2 mb-1.5 md:mb-3">
                        <div className="w-6 h-6 md:w-8 md:h-8 rounded-lg bg-emerald-50 flex items-center justify-center">
                          <Cpu size={14} className="md:size-[16px] text-emerald-500" />
                        </div>
                        <span className="text-[10px] md:text-sm text-gray-500 dark:text-slate-400 font-bold uppercase tracking-tight truncate">분배 완료</span>
                      </div>
                      <p className="text-base md:text-2xl font-black text-emerald-600 tabular-nums">{tenantDetail?.distributedCount ?? 0}<span className="text-[10px] md:text-sm ml-0.5 md:ml-1 text-gray-400 dark:text-slate-500">대</span></p>
                      <p className="hidden md:block text-xs text-gray-400 dark:text-slate-500 mt-1">프로젝트 지급 리소스</p>
                    </div>
                    <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm card-depth p-3 md:p-5 transition-all hover:shadow-md">
                      <div className="flex items-center gap-2 mb-1.5 md:mb-3">
                        <div className="w-6 h-6 md:w-8 md:h-8 rounded-lg bg-amber-50 flex items-center justify-center">
                          <Server size={14} className="md:size-[16px] text-amber-500" />
                        </div>
                        <span className="text-[10px] md:text-sm text-gray-500 dark:text-slate-400 font-bold uppercase tracking-tight truncate">잔여 Pool</span>
                      </div>
                      <p className="text-base md:text-2xl font-black text-amber-600 tabular-nums">{tenantDetail?.poolCount ?? 0}<span className="text-[10px] md:text-sm ml-0.5 md:ml-1 text-gray-400 dark:text-slate-500">대</span></p>
                      <p className="hidden md:block text-xs text-gray-400 dark:text-slate-500 mt-1">분배 가능 유휴 자원</p>
                    </div>
                  </>
                ) : (
                  <>
                    <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm card-depth p-3 md:p-5">
                      <div className="flex items-center gap-2 mb-1.5 md:mb-3">
                        <div className="w-6 h-6 md:w-8 md:h-8 rounded-lg bg-blue-50 flex items-center justify-center">
                          <Building2 size={14} className="md:size-[16px] text-blue-500" />
                        </div>
                        <span className="text-[10px] md:text-sm text-gray-500 dark:text-slate-400 font-bold uppercase tracking-tight truncate">전체 인프라</span>
                      </div>
                      <p className="text-base md:text-2xl font-black text-gray-900 dark:text-slate-100 tabular-nums">{totalNodes}<span className="text-[10px] md:text-sm ml-0.5 md:ml-1 text-gray-400 dark:text-slate-500">대</span></p>
                      <p className="hidden md:block text-xs text-gray-400 dark:text-slate-500 mt-1">DC 전체 관리 상한</p>
                    </div>
                    <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm card-depth p-3 md:p-5">
                      <div className="flex items-center gap-2 mb-1.5 md:mb-3">
                        <div className="w-6 h-6 md:w-8 md:h-8 rounded-lg bg-blue-50 flex items-center justify-center">
                          <Cpu size={14} className="md:size-[16px] text-blue-500" />
                        </div>
                        <span className="text-[10px] md:text-sm text-gray-500 dark:text-slate-400 font-bold uppercase tracking-tight truncate">Tenant 할당</span>
                      </div>
                      <p className="text-base md:text-2xl font-black text-blue-600 tabular-nums">{totalAllocated}<span className="text-[10px] md:text-sm ml-0.5 md:ml-1 text-gray-400 dark:text-slate-500">대</span></p>
                      <p className="hidden md:block text-xs text-gray-400 dark:text-slate-500 mt-1">배분 전역 누적 자원</p>
                    </div>
                    <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm card-depth p-3 md:p-5">
                      <div className="flex items-center gap-2 mb-1.5 md:mb-3">
                        <div className="w-6 h-6 md:w-8 md:h-8 rounded-lg bg-amber-50 flex items-center justify-center">
                        ...
                        <span className="text-[10px] md:text-sm text-gray-500 dark:text-slate-400 font-bold uppercase tracking-tight truncate">공용 잔여 풀</span>
                      </div>
                      <p className="text-base md:text-2xl font-black text-amber-600 tabular-nums">{totalNodes - totalAllocated}<span className="text-[10px] md:text-sm ml-0.5 md:ml-1 text-gray-400 dark:text-slate-500">대</span></p>
                      <p className="hidden md:block text-xs text-gray-400 dark:text-slate-500 mt-1">할당 대기 리소스</p>
                    </div>
                  </>
                )}
              </div>
              ...
              {selectedTenant && (
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white dark:bg-slate-800 p-3 sm:p-4 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm card-depth gap-3">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-blue-50 flex items-center justify-center">
                      <Building2 size={16} className="sm:size-[20px] text-blue-600" />
                    </div>
                    <div>
                      <h2 className="text-base sm:text-lg font-black text-gray-900 dark:text-slate-100">{selectedTenant.name} 상세</h2>
                      <div className="flex items-center gap-4 mt-0.5">
                        <span className="text-[10px] sm:text-xs font-bold text-gray-400 dark:text-slate-500 uppercase tracking-tighter flex items-center gap-1.5">
                          <div className="w-1.5 h-1.5 rounded-full bg-blue-500" /> 할당 범위: <span className="text-gray-600 dark:text-slate-400 font-mono tracking-normal">{tenantDetail?.tenantRangeText || '없음'}</span>
                        </span>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 w-full sm:w-auto">
                    {isAdmin && (
                      <button
                        ...
                        className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 px-3 py-1.5 sm:px-4 sm:py-2 bg-blue-600 rounded-lg text-[11px] sm:text-xs font-black uppercase tracking-wider text-white shadow-sm card-depth hover:bg-blue-700 transition-all active:scale-[0.98]"
                      >
                        <Server size={14} /> <span className="hidden sm:inline">할당 추가</span><span className="sm:hidden">할당</span>
                      </button>
                    )}
                    {isAdmin && (tenantDetail?.allocations.length || 0) > 0 && (
                      <button
                        onClick={() => setTenantReclaimTarget(selectedTenant)}
                        className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 px-3 py-1.5 sm:px-4 sm:py-2 border border-red-200 rounded-lg text-[11px] sm:text-xs font-black uppercase tracking-wider text-red-600 hover:bg-red-50 transition-all"
                      >
                        <Trash2 size={14} /> <span className="hidden sm:inline">자원 회수</span><span className="sm:hidden">회수</span>
                      </button>
                    )}
                  </div>
                </div>
              ...
              <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm card-depth overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-100 dark:border-slate-700 bg-gray-50 dark:bg-slate-900/30 flex justify-between items-center">
                  <h3 className="text-sm font-black text-gray-900 dark:text-slate-100 uppercase tracking-widest flex items-center gap-2">
                    {selectedTenant ? <Package size={16} className="text-blue-500" /> : <Server size={16} className="text-gray-500 dark:text-slate-400" />}
                  ...
                  {selectedTenant && (
                     <div className="px-3 py-1 rounded-full bg-amber-50 border border-amber-100 text-[10px] font-black text-amber-700 uppercase">
                       {tenantDetail?.poolCount}대 ( {tenantDetail?.poolRangeText} )
                     </div>
                  )}
                </div>
                   ...
                   <div className="md:hidden space-y-3 p-4 bg-gray-50 dark:bg-slate-900/50">
                      {selectedTenant ? (
                        (tenantDetail?.subtenants.length || 0) === 0 ? (
                          <div className="py-12 text-center text-sm text-gray-400 dark:text-slate-500 italic">등록된 하위 프로젝트가 없습니다.</div>
                        ) : (
                          tenantDetail?.subtenants.map(sub => (
                            <div key={sub.id} className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm card-depth p-4 space-y-4">
                              <div className="flex justify-between items-start">
                                <p className="text-sm font-black text-gray-900 dark:text-slate-100">{sub.name}</p>
                                {sub.count > 0 ? (
                                  ...
                                  <span className="px-2 py-0.5 rounded-full bg-gray-100 text-gray-400 dark:text-slate-500 text-[10px] font-black tracking-tight uppercase">0 Node</span>
                                )}
                              </div>
                              <div className="space-y-1">
                                <p className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest">인스턴스 구간</p>
                                <p className="font-mono text-xs text-gray-600 dark:text-slate-400">{sub.rangeText || '-'}</p>
                              </div>
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2 flex-1">
                                  ...
                                  <span className="text-[10px] font-black text-gray-400 dark:text-slate-500 tabular-nums">{Math.round(sub.usageRatio)}%</span>
                                </div>
                        ...
                        overviewRows.map((tenant, idx) => (
                          <div key={tenant.id} className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm card-depth p-4 space-y-4" onClick={() => setSelectedTenantIndex(idx + 1)}>
                            <div className="flex justify-between items-start">
                              <p className="text-sm font-black text-gray-900 dark:text-slate-100">{tenant.name}</p>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              <div>
                                <p className="text-[9px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest mb-1">할당/계약</p>
                                <p className="text-xs font-bold text-gray-900 dark:text-slate-100">{tenant.assignedCount} / {tenant.contractCount}대</p>
                              </div>
                              <div>
                                <p className="text-[9px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest mb-1">인스턴스 구간</p>
                                <p className="font-mono text-[10px] text-gray-500 dark:text-slate-400 overflow-hidden text-ellipsis whitespace-nowrap">{tenant.rangeText || '-'}</p>
                              </div>
                            </div>
                            <div className="flex gap-2">
                              <button onClick={(e) => { e.stopPropagation(); setSelectedTenantIndex(idx + 1); }} className="flex-1 py-2 bg-gray-50 dark:bg-slate-900 rounded-lg text-[10px] font-black text-gray-600 dark:text-slate-400">DETAIL</button>
                              {isAdmin && (
                   ...
                   <table className="hidden md:table w-full">
                      <thead>
                        <tr className="border-b border-gray-200 dark:border-slate-700">
                          <th className="px-6 py-4 text-[11px] font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider text-left bg-gray-50 dark:bg-slate-900/50">
                            {selectedTenant ? 'Subtenant 명' : 'Tenant 명'}
                          </th>
                          <th className="px-6 py-4 text-[11px] font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider text-center bg-gray-50 dark:bg-slate-900/50">
                            {selectedTenant ? '분계 (대)' : '계약/할당'}
                          </th>
                          <th className="px-6 py-4 text-[11px] font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider text-left bg-gray-50 dark:bg-slate-900/50">인스턴스 구간</th>
                          {selectedTenant ? (
                            <th className="px-6 py-4 text-[11px] font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider text-left bg-gray-50 dark:bg-slate-900/50">비율</th>
                          ) : (
                            <th className="px-6 py-4 text-[11px] font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider text-left bg-gray-50 dark:bg-slate-900/50">상태</th>
                          )}
                          <th className="px-6 py-4 text-[11px] font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider text-right bg-gray-50 dark:bg-slate-900/50">관리</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-100">
                        {selectedTenant ? (
                          (tenantDetail?.subtenants.length || 0) === 0 ? (
                            <tr><td colSpan={5} className="py-12 text-center text-sm text-gray-400 dark:text-slate-500 italic">등록된 하위 프로젝트가 없습니다.</td></tr>
                          ) : (
                            tenantDetail?.subtenants.map(sub => (
                              <tr key={sub.id} className="hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-900/50 transition-all">
                                <td className="px-6 py-4 font-bold text-gray-900 dark:text-slate-100 text-sm">{sub.name}</td>
                                <td className="px-6 text-center tabular-nums">
                                  {sub.count > 0 ? (
                                    ...
                                    <span className="px-2 py-0.5 rounded-full bg-gray-100 text-gray-400 dark:text-slate-500 text-xs font-bold">0대</span>
                                  )}
                                </td>
                                <td className="px-6 font-mono text-[11px] text-gray-500 dark:text-slate-400">{sub.rangeText || '-'}</td>
                          ...
                          overviewRows.map((tenant, idx) => (
                            <tr key={tenant.id} className="hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-900/50 transition-all">
                              <td 
                                onClick={() => setSelectedTenantIndex(idx + 1)}
                                className="px-6 py-4 font-bold text-gray-900 dark:text-slate-100 text-sm hover:text-blue-600 cursor-pointer"
                              >
                                {tenant.name}
                              </td>
                              <td className="px-6 text-center tabular-nums">
                                <div className="flex flex-col">
                                  <span className="text-xs font-bold text-gray-900 dark:text-slate-100">{tenant.assignedCount}대</span>
                                  <span className="text-[10px] text-gray-400 dark:text-slate-500">계약: {tenant.contractCount}대</span>
                                </div>
                              </td>
                              <td className="px-6 font-mono text-[11px] text-gray-500 dark:text-slate-400">{tenant.rangeText || '-'}</td>
                              <td className="px-6">
                              ...
                              <td className="px-6 py-4 text-right">
                                <div className="flex justify-end gap-2">
                                  <button 
                                    onClick={() => setSelectedTenantIndex(idx + 1)}
                                    className="px-3 py-1.5 rounded-lg text-[11px] font-black uppercase tracking-wider text-gray-600 dark:text-slate-400 hover:bg-gray-100 transition-all font-black"
                                  >
                                    Detail
                                  </button>
                                  {isAdmin && (
